  ...
  return (
    <div style={fullPageBackground}>
      <div style={loginCard}>
        ...
        {step === 0 && (
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={inputGroupStyle}>
              ...
              {/* CORRECCIÓN FINAL DEL CONTENEDOR */}
        ...
        {/* ... (Resto de los pasos Step 1 y Step 2 se mantienen igual) ... */}
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={inputGroupStyle}>
              <label style={labelStyle}>Ingresa tu usuario para comenzar</label>
              <input type="text" value={recoveryData.username} onChange={e => setRecoveryData({...recoveryData, username: e.target.value})} style={inputStyle} placeholder="Usuario..." />
            </div>
            <button onClick={handleFetchQuestion} disabled={loading} style={buttonStyle}>{loading ? "BUSCANDO..." : "CONTINUAR"}</button>
        ...
        {step === 2 && (
          <form onSubmit={handleResetPassword} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <div style={inputGroupStyle}>
              ...
              <input type="text" value={recoveryData.answer} onChange={e => setRecoveryData({...recoveryData, answer: e.target.value})} style={inputStyle} required />
            </div>
            <div style={inputGroupStyle}>
              <label style={labelStyle}>Nueva Contraseña</label>
              <input type="password" value={recoveryData.newPassword} onChange={e => setRecoveryData({...recoveryData, newPassword: e.target.value})} style={inputStyle} required />
            </div>
            <button type="submit" disabled={loading} style={{ ...buttonStyle, backgroundColor: '#3498db' }}>{loading ? "PROCESANDO..." : "CAMBIAR CONTRASEÑA"}</button>
            <button type="button" onClick={() => setStep(1)} style={backBtnStyle}>Atrás</button>
          </form>
        )}

        <div style={footerStyle}>© 2026 Todos los derechos reservados</div>
      </div>
    </div>
  );
}
...
const loginCard = { 
  width: '90%', maxWidth: '420px', padding: '40px 30px', 
  background: '#ffffff', borderRadius: '8px', 
  boxShadow: '0 15px 35px rgba(0,0,0,0.3)', boxSizing: 'border-box' 
};

// CONTENEDOR DEL PASSWORD: Forzamos el relative y aislamos del flex del index.css
const passwordWrapperStyle = {
  position: 'relative',
  width: '100%',
  display: 'block' // Cambiado de flex a block para evitar que el index.css lo estire raro
};

const passwordInputStyle = {
  width: '100%',
  padding: '12px 45px 12px 15px', // Padding derecho para que el texto no choque con el ojo
  borderRadius: '4px',
...
const logoPlaceholder = { width: '100px', height: '100px', borderRadius: '50%', background: '#f8f9fa', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #eee' };
...
const buttonStyle = { marginTop: '15px', padding: '14px', color: 'white', border: 'none', borderRadius: '4px', fontSize: '1em', fontWeight: 'bold', cursor: 'pointer' };
